...
<Select.Root type="single" bind:value={ value } { disabled } { required }>
	<div class="flex flex-col gap-1.5 w-full">
		...
		<div class="relative w-full">
			<Select.Trigger
				class="w-full px-4 py-2.5 rounded-xl border transition-all duration-200 flex items-center justify-between text-left
			...
			<Select.Content
				class="z-50 rounded-2xl border border-(--border) bg-(--bg-surface) p-1.5 shadow-(--shadow-lg) animate-in fade-in duration-200 w-[var(--bits-select-anchor-width)] min-w-[12rem]"
				sideOffset={ 4 }
			>
				{#each options as opt}
					<Select.Item
