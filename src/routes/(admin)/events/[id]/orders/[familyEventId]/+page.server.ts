...
export const load: PageServerLoad = async ( event ) => {
	...
	const { id, familyEventId } = event.params;
...
export const actions: Actions = {
	createOrder: async ( { request, params } ) => {
		...
		const eventProducts = await getEventProducts( params.id );
		...
		for ( const ep of eventProducts ) {
			if ( [ 'OUT_OF_STOCK', 'PAUSED', 'DISCONTINUED' ].includes( ep.status ) ) {
				continue; // Skip products not available
			}
		...
		if ( itemsToCreate.length === 0 ) {
			return fail( 400, { error: 'Debes ingresar al menos una porción para crear la orden.' } );
		}
		...
		const scannedByUserId = adminUser?.id ?? null;

		const orderBody = {
			event_id           : params.id,
			family_event_id    : params.familyEventId,
			scanned_by_user_id : scannedByUserId,
			status             : 'COMPLETED' as const, // Handed over directly by admin
